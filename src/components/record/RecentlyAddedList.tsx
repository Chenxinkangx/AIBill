    ...
    <section className="space-y-2" aria-labelledby="recently-added-title">
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-2">
          <h2 id="recently-added-title" className="text-sm font-medium text-gray-600">
            刚刚添加
          </h2>
        ...
        <button
          type="button"
          onClick={onViewAll}
          className="text-xs font-medium text-indigo-600 hover:text-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2"
        >
          查看全部账单
        </button>
      </div>

      <div className="overflow-hidden rounded-xl bg-white shadow-sm shadow-gray-100/80">
              ...
              <div key={record.id} className="flex items-start justify-between gap-3 px-4 py-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="shrink-0 text-[10px] font-medium text-emerald-600">已保存</span>
                  </div>
                  ...
                  {recordTags.length > 0 && (
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {recordTags.map((tag) => (
                        <span key={tag.id} className="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] text-gray-600">
                          {tag.name}
                        </span>
                      ))}
                    </div>
                ...
                <span className={`shrink-0 text-sm font-semibold tabular-nums ${record.type === 'income' ? 'text-emerald-600' : 'text-gray-900'}`}>
        ...
        {hiddenCount > 0 && (
          <button
            type="button"
            onClick={onViewAll}
            className="w-full border-t border-gray-100 px-4 py-2.5 text-center text-xs font-medium text-gray-600 transition-colors hover:bg-gray-50"
          >
